...
constructor(private val certs: CertificateService, private val partners: PartnerService) : PipelineTask<Context> {
    ...
    override fun process(ctx: Context) {
        ...
        when (p!!.sendSettings.mdnMode) {
            MdnMode.ASYNC -> MessageBus.publishAsync(SavePendingMdnMsg(ctx.message))
            MdnMode.STANDARD -> receiveMdn(ctx.message, ctx.response!!)
            else -> logger.debug("No MDN was requested (" + p.sendSettings.mdnMode + ")")
        }
    }

    @Throws(IOException::class, AS2Exception::class, DispositionException::class, GeneralSecurityException::class)
    private fun receiveMdn(message: OutgoingFileMessage, response: HttpResponse) {

        val mdn = IncomingSyncMdn(message, response)
        ...
        if (response.entity == null) {
            ...
            throw HttpResponseException(response.statusLine.statusCode, "Expected an MDN reply (non-async), but body was empty")
        }

        try {
            var mimeData = MimeUtil.fromHttpResponse(response)

            // if the mdn is signed, first verify the signature and strip it, before using that to populate
            if (MimeUtil.isSigned(mimeData)) {
                val senderCert = certs.getCertificate(mdn.responseHeaders["AS2-From"]!!)
            ...
            MessageBus.publish(SaveMdnMsg(mdn))
            ...
            ServerEvents.mdnReceiveError(message, ex, "Remote server reported an error receiving the message: " + (ex.disposition?.dispositionDescription ?: "<empty>"))
            throw ex
            ...
            ServerEvents.mdnReceiveError(message, ex, "Generic exception receiving MDN: " + ex.localizedMessage)
            throw AS2Exception("Generic exception processing the MDN", ex)
        } finally {
            ServerEvents.mdnReceiveEnd(message)
        }
    }
    ...
    companion object {
        private val logger = LogFactory.getLog(ReceiveMdn::class.java.simpleName)
    }
}
