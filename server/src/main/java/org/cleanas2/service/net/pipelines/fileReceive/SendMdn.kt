...
constructor(private val certs: CertificateService, private val partners: PartnerService) : PipelineTask<Context> {
    ...
    override fun process(ctx: Context) {
        try {
            ...
            if (ctx.fileMessage!!.mdnMode == NONE) {
                ctx.httpResponse.setStatusCode(HttpStatus.SC_OK)
                ctx.httpResponse.entity = StringEntity(format("File Received Ok, NO MDN was requested"))
                return
            }

            // incoming message, so the sender is our partner.
            val partner = this.partners.getPartner(ctx.fileMessage!!.senderId)
            val mdn = MdnUtil.createReplyMdn(
                    ctx.fileMessage!!,
            ...
            when (ctx.fileMessage!!.mdnMode) {
                STANDARD -> {
                    MessageBus.publishAsync(SaveIncomingMdnMsg(mdn))
                ...
                ASYNC ->
                    ...
                    MessageBus.publishAsync(SendAsyncMdnMsg(mdn))
                NONE -> TODO()
            }
        } finally {
    ...
    private fun sendMdnOnSameConnection(response: HttpResponse, mdn: ReplyMdn) {
        try {
            ...
            if (mdn.isSignedReply) {
                try {
                    val certificate = certs.getCertificate(mdn.companyId)
                    val privateKey = certs.getPrivateKey(mdn.companyId)
            ...
            response.setHeader("Content-Type", outBody.contentType)
            response.entity = InputStreamEntity(outBody.inputStream)
    ...
    companion object {
        private val logger = LogFactory.getLog(SendMdn::class.java.simpleName)
    }
